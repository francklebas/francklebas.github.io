...
const approachBullets = computed(
  () => content.value.home.approachBullets || content.value.about.philosophy,
);

const metrics = computed(() =>
  content.value.home.metrics.map((metric) => {
    const parsed = Number.parseInt(metric.value.replace(/[^0-9]/g, ""), 10);
    const fallbackProgress = Number.isFinite(parsed)
      ? Math.min(parsed, 100)
      : 60;
    return {
      ...metric,
...
const calendlyHref = computed(
  () => config.public.calendlyUrl || content.value.contact.calendly || "",
);

const heroContactHref = computed(
  () => calendlyHref.value || heroMailtoHref.value,
);
...
<template>
  <section class="page" v-if="content">
    <p class="eyebrow badge badge-outline reveal-init">
      {{ content.home.hero.eyebrow[locale as "en" | "fr" | "sv"] }}
    </p>
    <div class="hero-grid">
      <div class="reveal-init">
        <h1>{{ content.home.hero.title[locale as "en" | "fr" | "sv"] }}</h1>
        <p class="lead">
          {{ content.home.hero.lead[locale as "en" | "fr" | "sv"] }}
        </p>

        <div class="hero-actions reveal-init" style="--reveal-delay: 80ms">
          <NuxtLink
            class="btn btn-primary"
            :to="localePath(content.home.hero.primaryCtaHref)"
          >
            {{
    ...
    <div
      class="section"
      v-motion="'parallax-highlights'"
      :initial="{ opacity: 0 }"
      :visible-once="{ opacity: 1, transition: { duration: 700, delay: 170 } }"
    >
    ...
    <div
      class="section"
      v-motion="'parallax-approach'"
      :initial="{ opacity: 0 }"
      :visible-once="{ opacity: 1, transition: { duration: 700, delay: 220 } }"
    >
      <h2>
        {{
          content.home.approachTitle?.[locale as "en" | "fr" | "sv"] ||
          t("ui.myApproach")
        }}
      </h2>
      <div class="card bg-base-100 border border-base-content/15">
        <div class="card-body p-5">
          <p v-for="paragraph in approachIntro" :key="paragraph.en">
            {{ paragraph[locale as "en" | "fr" | "sv"] }}
          </p>
          <ul class="approach-list">
            <li v-for="item in approachBullets" :key="item.en">
              {{ item[locale as "en" | "fr" | "sv"] }}
            </li>
          </ul>
          <p class="muted">
            {{
    ...
    <div
      ...
      :initial="{ opacity: 0 }"
      :visible-once="{ opacity: 1, transition: { duration: 700, delay: 280 } }"
    >
      ...
      <div class="grid cols-2">
        ...
        >
          <div class="card-body p-5 metric-card">
            ...
            <p class="muted">
              {{ metric.label[locale as "en" | "fr" | "sv"] }}
            </p>
          </div>
        </article>
      </div>
    ...
    <div
      class="section"
      v-motion="'parallax-featured'"
      :initial="{ opacity: 0 }"
      :visible-once="{ opacity: 1, transition: { duration: 700, delay: 360 } }"
    >
      <div class="section-head">
        <h2>{{ t("ui.featuredWork") }}</h2>
        <NuxtLink class="btn btn-sm btn-ghost" :to="localePath('/projects')">{{
        ...
        >
          ...
          <div class="card-body p-5 flex flex-col gap-4">
            <div class="flex-1 space-y-3">
              <a :href="project.repoUrl" target="_blank" rel="noopener noreferrer" class="hover:underline">
                <h3 class="text-secondary text-lg">{{ project.name }}</h3>
              </a>
              <p v-if="project.role" class="badge badge-ghost badge-sm w-fit">
                {{ project.role[locale as "en" | "fr" | "sv"] }}
              </p>
              <p>{{ project.useCase[locale as "en" | "fr" | "sv"] }}</p>
              <p class="muted">
                {{ project.impact[locale as "en" | "fr" | "sv"] }}
              </p>
            </div>
